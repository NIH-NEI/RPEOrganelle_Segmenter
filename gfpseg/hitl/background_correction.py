...
from gfpseg.segmentation.segment_GFP import get_stack_channel
...
for f in os.listdir(maindirpath):
    ...
    if savesegmentation:
        OmeTiffWriter.save(data=IMG_tophat, uri=f'{savedirpath}{bn}_wtophat.tiff', compression='zlib')
        OmeTiffWriter.save(data=IMG_tophat, uri=f'{savedirpath}{bn}_btophat.tiff', compression='zlib')
        OmeTiffWriter.save(data=IMG_clahe, uri=f'{savedirpath}{bn}_clahe.tiff', compression='zlib')
        OmeTiffWriter.save(data=IMG, uri=f'{savedirpath}{f}', compression='zlib')
    if plot_histograms:
