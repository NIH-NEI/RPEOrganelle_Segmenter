...
def getscaleparameters(mean_pixel_dimension=None, frac_deviation=0.5):
    """
    Calculates a list of scale parameters based on a mean pixel dimension and a fractional deviation.
    The fractional deviation is used to generate a range of scale parameters around the mean.
    ...
    Returns:
        list: list of three scale parameters: [mean * (1 - frac_deviation), mean, mean * (1 + frac_deviation)].
    """
...
def returnscalecutofflists(scales, cutoffs, paramname="f2params", secondparamvals=None):
    """
    Generates a list of parameter dictionaries combining scales and cutoffs.
    Args:
        :param scales (list): List of scale values.
        :param cutoffs (list): List of cutoff values.
        :param param_name (str or list): The parameter name(s) to use in the dictionary. Can be a single string or a list of two strings.\
                                        Must be f2/s2/f3/s3+params.
        :param second_param_vals (list, optional): List of values for a second parameter, if applicable.
...
# Mapping of directory names to channel names
channels = {
    "LaminB": "LMNB",
    "LAMP1": "LAMP1",
    "Sec61": "SEC",
...
# Mapping of directory names to segmentation functions
usefunction = {
...
def main():
    parser = argparse.ArgumentParser(description="Segment GFP stacks with HITL parameter selection.")
    parser.add_argument("--phase", type=int, default=4, help="Phase of the segmentation process.")
    parser.add_argument("--input_dir", type=str, default="../Results/../Stacks_hotl/",
                        help="Path to the input directory.")
    parser.add_argument("--output_dir", type=str, default="../Results/../stgaltest/",
                        help="Path to the output directory.")
    parser.add_argument("--do_channels", nargs='+', default=["ST6GAL1"], help="Channels to process.")
    args = parser.parse_args()

    phase = args.phase
    dochannels = args.do_channels
    maindirpath = args.input_dir
    savedirpath = args.output_dir
    maindirs = listdir(maindirpath)
    # savedirs = listdir(savedirpath)

    # Calculation of fractional deviation based on phase
    fracdev = 0.5 / (2 ** (phase - 1))  # for generation of values for each phase

    # PHASE 1
    if phase == 1:
        cutoffparams = [0.01, 0.05, 0.1]
        meanpixels = {
            "LaminB": 3,
            "LAMP1": 3.5,
            "Sec61": 3,
    ...
    else:  # More phases can be added if necessary
        exit()
    ...
    for i, dirname in enumerate(maindirs):
        ...
        dictofparams[dirname] = returnscalecutofflists(getscaleparameters(meanpixels[dirname], frac_deviation=fracdev),
                                                       cutoffparams, paramname=paramtypes[dirname],
                                                       secondparamvals=spvals)
