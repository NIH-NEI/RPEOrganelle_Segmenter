...
from .vessel import filament_2d_wrapper, filament_3d_wrapper
...
def savesegmented(data, savepath, basename, parametertype=None, postprocessinginfo=None, useparameterinfo=True):
    ...
    OmeTiffWriter.save(data=data, uri=filename, compression='zlib')
...
def segmentsec61tacks(fpath, savepath, params, channel="sec61b", minarea=None):
    ...
    tifffile.imwrite(f'{savepath}{name}{s}_GFP.tiff', out, compression='zlib')
...
def segmentlaminstacks(fpath, savepath, params, channel="lmnb1", minarea=None, padsize=0):
    ...
    # name = fpath.split("/")[-1].split(".")[0]
    # print(name)
    tifffile.imwrite(f'{savepath}{fname}{methodinfo}_GFP.tiff', out, compression='zlib')
    # writer.save(out)
...
def segmenttom(fpath, savepath, params, channel="tom20", minarea=None):
    ...
    if not os.path.exists(savepath):
        print(savepath)
        os.mkdir(savepath)
    tifffile.imsave(f'{savepath}{fname}_f2{s}_GFP.tiff', out, compression='zlib')
    # writer.save(out)
...
def segmentlampstacks(fpath, savepath, params, channel="lamp1", minarea=None):  # DO NOT USE FILAMENTS FOR NOW
    ...
    seg = remove_small_objects(bw_fill > 0, min_size=minarea, connectivity=1)
    seg = seg > 0
    out = seg.astype(np.uint8)
    out[out > 0] = 255
    ################################
    ...
    tifffile.imwrite(f'{savepath}{fname}s2{ss}_f2{sf}_GFP.tiff', out, compression='zlib')
    # writer.save(out)
...
def segmentstgal(fpath, savepath, params, channel="st6gal1", method="nomo", minarea=None, thin_dist=2):
    ...
    seg = remove_small_objects(bw_combine > 0, min_size=minarea, connectivity=1)
    ...
    name = fpath.split("/")[-1].split(".")[0]
    if method == "nomo":
        tifffile.imwrite(f'{savepath}{name}_s3{s3p}_GFP.tif', out, compression='zlib')
    else:
        tifffile.imwrite(f'{savepath}{name}_thin{thin_dist_preserve}_method{method}_s3{s3p}_GFP.tif', out, compression='zlib')
    # writer.save(out)
...
def segmentfbl(fpath, savepath, params, channel="fbl", minarea=None):
    ...
    final_seg = remove_small_objects(bw > 0, min_size=minarea, connectivity=1)
    final_seg = final_seg > 0
    out = final_seg.astype(np.uint8)
    out[out > 0] = 255

    OmeTiffWriter.save(data=out, uri=f'{savepath}{name}_s2{s2p}_GFP.tiff', compression='zlib')
...
def segmenttub(fpath, savepath, params, channel="tuba1b", minarea=None):
    ...
    OmeTiffWriter.save(data=out, uri=f'{savepath}{name}_f3{f3p}_GFP.tiff', compression='zlib')
...
def segmentrab5(fpath, savepath, params, channel="rab5", minarea=None):
    ...
    bw_fill = hole_filling(bw, 0, fill_max_size, fill_2d)
    # bw_fill = bw
    final_seg = remove_small_objects(bw_fill > 0, min_size=minarea, connectivity=1)
...
def segmentdsp(fpath, savepath, params, channel="dsp", minarea=None, method="3dspot", savesegmentation=True,
    ...
    # watershed
    maskminarea = minarea
    Mask = remove_small_objects(bw > 0, min_size=maskminarea, connectivity=1)
...
def segmentcetn2(fpath, savepath, params, channel="cetn2", minarea=None):
    ...
    OmeTiffWriter.save(data=out, uri=f'{savepath}{name}_s3{s3p}_GFP.tiff', compression='zlib')
