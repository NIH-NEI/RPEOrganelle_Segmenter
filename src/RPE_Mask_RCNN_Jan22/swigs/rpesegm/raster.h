...
public:
	...
	int countColor(Particle &ptc, unsigned char c);
...
protected:
	...
	int otsu;
	//
	NbrPoint *step_disp = NULL;
	double step;
	//
	int previous_angle(std::vector<Point> &uppath, int back=1) {
		int prev_idx = int(uppath.size()-1) - back;
		if (prev_idx < 0) prev_idx = 0;
		Point &p = uppath[prev_idx];
		return ang.value(p.x, p.y);
	}
	bool is_at_border(int x, int y, int dist=3) {
		return (x < dist || x >= img.w-dist || y < dist || y >= img.h-dist);
	}
	virtual bool veto_advance(int x, int y) { return false; }

public:
	GradientWalker(Raster16 &_img, Raster8 &_ang, Raster8 &_out, int _otsu) :
		img(_img), ang(_ang), out(_out), otsu(_otsu) {
		step_disp = new NbrPoint[AUNITMAX];
		setStep(5.);
	}
	virtual ~GradientWalker() {
		delete [] step_disp;
	}
	//
	...
	virtual void walk_one(std::vector<Point> &uppath, int ortho, int _bend);
	int walk(std::vector<Point> &path, int x0, int y0);
	bool close_path(std::vector<Point> &path);
};

class DnaGradientWalker : public GradientWalker
{
public:
	int zcount = 0;
protected:
	...
	virtual bool veto_advance(int x, int y) override {
		if (!msk || zcount==0) return false;
		return msk->countColors(x, y, HOOD_SIZE_RAD3, 0) < zcount;
	}
public:
	DnaGradientWalker(Raster16 &_img, Raster8 &_ang, Raster8 &_out, int _otsu) :
		GradientWalker(_img, _ang, _out, _otsu)	{
	}
	void set_zero_mask(Raster8 *msk, int zcount) {
		this->msk = msk;
		this->zcount = zcount;
	}

	virtual void walk_one(std::vector<Point> &uppath, int ortho, int _bend) override;
};
