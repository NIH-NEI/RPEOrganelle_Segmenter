...
from src.RPE_Mask_RCNN_Feb22.rpecontour import *
from src.RPE_Mask_RCNN_Feb22.util import *
...
# class RpeMultiTifStack(object):
#     def __init__(self, fpath):
#         self.fpath = fpath
#         #
#         self.basedir = os.path.abspath(os.path.dirname(fpath))
#         #
#         mm = self.decode_multistack(fpath)
#     #
#     @staticmethod
#     def decode_multistack(fpath):
#         try:
#             res = {}
#             basedir, fn = os.path.split(fpath)
#             bn, ext = os.path.splitext(fn)
#             assert ext.lower() in ('.tif', '.tiff')
#             idx = bn.rfind('.')
#             assert idx >= 0
#             basename = bn[:idx]
...
#                 bn, ext = os.path.splitext(fn)
#                 if not ext.lower() in ('.tif', '.tiff') or not bn.startswith(basename): continue
#                 idx = bn.rfind('.')
#                 chname = bn[idx:]
#                 res[chname] = fpath
#             for chname in ('DNA', 'Actin', 'Membrane'):
#                 assert chname in res
#             return res
...
class RpeTiffStack(object):
    def __init__(self, fpath, default_channel='DNA'):
        ...
        #
        self.rgbStackName = self.cache.getCacheRgbName()
        self.rgbReader = None
    #
    ...
    #
    def isRGB(self, chname):
        ichan = self.channelId(chname)
        if ichan < 0: return False
        chname = self.channelName(ichan)
        return chname in ('DNA', 'Actin', 'Membrane')
    #
    ...
    def getFrame(self, iframe, chname=None, nocache=False):
        ...
        return data
    #
    def hasRgbCache(self):
        return os.path.isfile(self.rgbStackName)
    #
    def _openRgbStack(self):
        if self.rgbReader is None:
            if not self.hasRgbCache():
                return False
            try:
                self.rgbReader = TiffFile(self.rgbStackName)
            except Exception as ex:
                print (ex)
                return False
        return True
    def getFrameRgb(self, iframe, nocache=False):
        chid = 'RGB'
        fr_key = (chid, iframe)
        if fr_key in self.framecache:
            return self.framecache[fr_key]
        self._openRgbStack()
        if not self.rgbReader is None:
            page = self.rgbReader.pages[iframe]
            data = page.asarray()
            if not nocache:
                self.framecache[fr_key] = data
            return data
    ...
    #
    def getFrameAndSegmentation(self, iframe, chname=None, rgb=False):
        if chname is None:
            chname = self.default_channel
        if self.isZ01(chname):
            key = 'Z01flat'
        ...
        fr_data = None
        if rgb:
            fr_data = self.getFrameRgb(iframe)
        if fr_data is None:
            fr_data = self.getFrame(iframe, chname)
        return fr_data, self.cache.getFrameContours(chname, iframe)
    #
...
class RpeCache(object):
    ...
    #
    def getCacheRgbName(self):
        return os.path.join(self.cache_dir, self.base_name+'_rgb.tif')
    #
...
if __name__ == '__main__':
    ...
    assert len(sys.argv) > 1, 'Need an .ome.tif or .rpe.json file as a parameterr'
    tifstk = RpeTiffStack(sys.argv[1])
    print(tifstk.base_name, tifstk.channels, tifstk.dimorder)
    
    # if len(sys.argv) < 2:
    #     print('Usage: python rpefs.py </path/to/meta.rpe.json> [/target/directory/]')
    #     sys.exit(0)
    # fpath = os.path.abspath(sys.argv[1])
    # assert os.path.isfile(fpath) and fpath.lower().endswith('.rpe.json'), \
    #     'First parameter must be an existing RPE Metadata (.rpe.json) file.'
    # if len(sys.argv) > 2:
    #     cdir = os.path.abspath(sys.argv[2])
    #     assert os.path.isdir(cdir), 'Second parameter (if any) must be an existing directory.'
    # else:
    #     cdir = os.path.dirname(fpath)
    #
    # tifstk = RpeTiffStack(sys.argv[1])
    # for chname in tifstk.channels:
    #     tgtpath = os.path.join(cdir, f'{tifstk.base_name}.{chname}.tif')
    #     print(chname, '->', tgtpath)
    #     imwrite(tgtpath, tifstk.getChannelData(chname), compress=6, photometric='minisblack')
