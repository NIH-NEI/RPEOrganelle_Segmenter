...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--raw', required=False, action="store_true",
            help="Write raw predictions (before 3d assembly) to .pkl")
    parser.add_argument('-l', '--cell-ids', required=False, type=int,
            metavar="0..5",
            default=-1,
            help="Generate _ids.tif after specified # of dilations (Actin only). Default: -1 (don't generate _ids.tif).")
    ...
    assert int(args.cell_ids) <= 5, 'Number of dilations for cell ids must not exceed 5.'
...
def predict(argv):
    if len(argv) > 1 and argv[1].startswith('@'):
        try:
            arglist = parse_resp_file(argv[1][1:])
        except Exception as ex:
            print('Error parsing response file:', str(ex))
            exit(1)
    else:
    ...
    # print(args)
    # sys.exit(0)
    from src.RPE_Mask_RCNN_Apr22 import predictors
